...
function resizeCanvas() {
   ...
   // has changed.
   var width  = realcanvas.clientWidth;
   var height = realcanvas.clientHeight;
   if (realcanvas.width != width ||
       realcanvas.height != height) {
     // Change the size of the canvas to match the size it's being displayed
     realcanvas.width = width;
     realcanvas.height = width;
   }
}

function initcanvas(canvasname,background)
{
    // Get A WebGL context
    var div = document.getElementById(canvasname);
    div.innerHTML = "<canvas id = \"" + "canvas" + canvasname  + "\" style=\"width:100%;height:100%;\"/>";
    var canvas = document.getElementById("canvas" + canvasname);
    realcanvas = canvas;
    resizeCanvas();
    
    // canvas.onmousedown = handleMouseDown;
    // canvas.onmouseup   = handleMouseDown;
    // canvas.onkeydown   = handleMouseDown;
    // canvas.onkeyup     = handleMouseDown;
    ...
    gl.canvasratio  = gl.canvas.width/gl.canvas.height;
